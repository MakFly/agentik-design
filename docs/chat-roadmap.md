...
## RAF — Reste à faire (backlog produit par surface)

> Légende : 🗣️ **Assistant** (Personal Assistant) · 🏢 **Platform** (Multica) ·
> 🔗 **Transverse** (les deux). Le détail actionnable de chaque item est dans les Phases
> plus bas (chaque titre de phase porte sa surface).

### 🗣️ Assistant — RAF
| Item | Réf. | Ce que ça implique |
|---|---|---|
| **Tool-calls / activité dans le thread** | Phase 2 | Étendre la **gateway** (`domains/chat/gateway.ts`) pour émettre des parts `tool-call` (aujourd'hui seulement `text`+`reasoning`) **et** le rejeu depuis l'historique. Couche = chat uniquement. |
| **Slash commands réels** dans le composer | Phase 3 | Remplacer les stubs `console.log` de `base.tsx` (`/new`, `/agent switch`…). Les commandes qui *mutent* (agents/tools/channels) consomment les APIs **plateforme**. |
...
| Item | Réf. | Ce que ça implique |
|---|---|---|
| **Manage drawer in-chat** (agents/tools/channels) | Phase 3 | Le chat (assistant) **héberge** des panneaux qui pilotent des domaines **plateforme** → couple explicitement les deux surfaces. |
| **Tests E2E** | Phase 4 | `chat.spec` + `nav.spec` (`/platform/*`) + Telegram sim : **non rejoués** depuis le split → exécuter + étendre. |
| **Discipline des liens** | — | Tout nouveau lien **doit** passer par `hrefFor` (le split a migré ~60 liens ; un lien en dur casse le préfixe `/platform`). |

### Décisions de périmètre à trancher (avant d'attaquer le RAF)
- **Automations / Cron** : la *gestion* des tâches cron vit côté **assistant** (« mes
  ...
  assistant mais le backend cron est de l'ops — incohérence à lever.
...
## Phase 3 — Gestion in-chat (agents / tools / channels) — 🔗 Transverse
...
## Phase 4 — Tests E2E (Playwright chat + Telegram sim) — 🔗 Transverse
