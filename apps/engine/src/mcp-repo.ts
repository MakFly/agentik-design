...
import { getCredentialDecrypted } from "./repo";
import type { CreateMcpServerInput, UpdateMcpServerInput } from "./mcp-schemas";
import type { McpTransport, ToolGrantRecord } from "./db/schema";

const { mcpServers, mcpTools, agents, agentVersions, agentTasks } = schema;
...
async function resolveAgentIdForInvocation(teamId: string, input: { agentId?: string; runId?: string }) {
  ...
  const [task] = await db
    ...
    .from(agentTasks)
    .where(and(eq(agentTasks.teamId, teamId), eq(agentTasks.id, input.runId)))
    .limit(1);
