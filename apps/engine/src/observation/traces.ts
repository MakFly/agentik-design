...
import { listRuns, getRunDetail, type WebRunStatus } from "../domains/runs";
...
async function loadRunForTrace(
  teamId: string,
  id: string,
): Promise<{ run: PRun; steps: PStep[] } | null> {
  const d = await getRunDetail(teamId, id);
...
export async function getTrace(teamId: string, id: string): Promise<{ trace: TraceSummary; spans: Span[]; services: string[] } | null> {
  const detail = await loadRunForTrace(teamId, id);
...
export async function listTraces(teamId: string, filters: TraceListFilters) {
  const runs = (await listRuns(teamId, {})) as unknown as PRun[];
