...
import type { ProjectResourceType, ProjectTaskPriority, ProjectTaskStatus, ProjectType } from "./db/schema";

const { projects, projectResources, projectTasks, projectTaskComments, projectWorkspaces, agentTasks, agents } = schema;
...
export async function updateProjectTask(
  ...
  const [task] = await db.select().from(projectTasks).where(and(eq(projectTasks.teamId, teamId), eq(projectTasks.id, taskId))).limit(1);
  if (!task) return null;
  ...
  const [updated] = await db.update(projectTasks).set(patch).where(and(eq(projectTasks.teamId, teamId), eq(projectTasks.id, taskId))).returning();
  await db.update(projects).set({ updatedAt: sql`now()` }).where(eq(projects.id, task.projectId));
  return updated ?? null;
}

export async function listProjectTaskComments(teamId: string, taskId: string) {
  return db
    .select()
    .from(projectTaskComments)
    .where(and(eq(projectTaskComments.teamId, teamId), eq(projectTaskComments.projectTaskId, taskId)))
    .orderBy(projectTaskComments.createdAt);
}

export async function addProjectTaskComment(teamId: string, taskId: string, userId: string, content: string) {
  ...
  const [comment] = await db
    ...
    .values({ id: genId("pmsg"), teamId, projectTaskId: taskId, authorKind: "user", userId, content: clean })
    .returning();
  await db.update(projectTasks).set({ updatedAt: sql`now()` }).where(eq(projectTasks.id, taskId));
  return { comment };
}
...
export async function runProjectTask(teamId: string, taskId: string, instruction?: string) {
  const [task] = await db.select().from(projectTasks).where(and(eq(projectTasks.teamId, teamId), eq(projectTasks.id, taskId))).limit(1);
  if (!task) return { error: "task_not_found" as const };
