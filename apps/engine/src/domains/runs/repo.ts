...
type DaemonRunRowDb = typeof runs.$inferSelect;
type AgentStatsRunRow = Pick<
  DaemonRunRowDb,
  "agentId" | "status" | "durationMs" | "createdAt"
>;
type RunMsgRowDb = typeof runMessages.$inferSelect;
...
function fallbackResultStep(task: DaemonRunRowDb, agentName?: string) {
...
function artifactsFromRun(task: DaemonRunRowDb) {
  const result =
 ...
 * one step (id = msg.id, index = msg.seq), mirroring runMessageToStep so the
...
/** Subset of apps/web RunEvent that daemon-run streams emit. See events.ts. */
type LiveRunEvent =
...
export async function getRunStatus(
  ...
  const [t] = await db
    ...
    .limit(1);
  return t ? t.status as WebRunStatus : null;
}
...
export async function getAgentRow(teamId: string, agentId: string) {
  ...
  const tasks = await db
    ...
    .from(runs)
    .where(
      and(eq(runs.teamId, teamId), eq(runs.agentId, agentId)),
    );
...
/** Tenancy-scoped run detail for workflow or daemon executors. */
