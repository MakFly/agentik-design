...
import { autoRetryTask, failTask } from "./daemon-repo";
...
export async function scanStaleTasks(): Promise<ScanResult> {
  return db.transaction(async (tx) => {
    ...
    const stale = (await tx.execute(sql`
      ...
      FROM agent_tasks
      WHERE (status = 'dispatched' AND dispatched_at < now() - interval '${sql.raw(DISPATCHED_TIMEOUT)}')
         OR (status = 'running'    AND started_at    < now() - interval '${sql.raw(RUNNING_TIMEOUT)}')
